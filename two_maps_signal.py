...
tecl = yf.download("TECL", start=warmup_start, end=future_end, progress=False, auto_adjust=False, actions=True, group_by="ticker")
...
tecl_close = get_close(tecl, "TECL")
...
tecl_close.index = normalize_index(tecl_close.index)

# Align to common index
common_idx = spy_close.index.intersection(smh_close.index).intersection(vgt_close.index).intersection(tecl_close.index)
spy_close = spy_close[common_idx]
smh_close = smh_close[common_idx]
vgt_close = vgt_close[common_idx]
tecl_close = tecl_close[common_idx]
...
tecl_close = tecl_close[tecl_close.index >= start_date]
...
tecl_ret = tecl_close.pct_change()
