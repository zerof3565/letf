#!/usr/bin/env python3
"""Backtest of the SPY 200SMA Two-Map Strategy using TQQQ, refreshing CSV first."""
...
# --- Parse arguments ---
parser = argparse.ArgumentParser(description="SPY 200SMA Two-Map Strategy Backtest (TQQQ)")
...
print("-" * 50)
cmd = ["python", "two_maps_signal_tqqq.py"]
if args.start_date:
...
# --- Load data ---
csv_file = "two_maps_signal_tqqq.csv"
...
# --- Map B: Crash mode (includes TQQQ) ---
...
for i in range(len(pct)):
    ...
    if mode_in_map_b:
        if pct[i] >= 10:
            mode_in_map_b = False
            smh_w, vgt_w, tqqq_w = get_weights_map_a(pct[i])
        else:
            smh_w, vgt_w, tqqq_w = get_weights_map_b(pct[i])
    else:
        if pct[i] <= -10:
            mode_in_map_b = True
            smh_w, vgt_w, tqqq_w = get_weights_map_b(pct[i])
...
strategy_ret = (
    ...
    pd.Series(df["allocation_TQQQ"].shift(1).values, index=df["date"]) / 100 * pd.Series(tqqq_ret, index=df["date"])
)
...
tqqq_equity = pd.Series((1 + pd.Series(tqqq_ret, index=df["date"])).cumprod(), index=df["date"])
...
for eq, nm in [(spy_equity, "Buy SPY"), (smh_equity, "Buy SMH"),
               (vgt_equity, "Buy VGT"), (tqqq_equity, "Buy TQQQ"),
...
for yr in range(spy_equity.index.year.min(), spy_equity.index.year.max() + 1):
    ...
    tqqq_y = tqqq_equity[tqqq_equity.index.year == yr]
    ...
    tqqq_ret_y = (tqqq_y.iloc[-1] / tqqq_y.iloc[0] - 1) * 100
    strat_ret_y = (strat_y.iloc[-1] / strat_y.iloc[0] - 1) * 100

    spy_yearly[yr] = spy_ret_y
    smh_yearly[yr] = smh_ret_y
    vgt_yearly[yr] = vgt_ret_y
    tqqq_yearly[yr] = tqqq_ret_y
    strat_yearly[yr] = strat_ret_y

print(f"{'Year':<8} {'SPY':>10} {'SMH':>10} {'VGT':>10} {'TQQQ':>10} {'Strategy':>12}")
print("-" * 70)
for yr in sorted(strat_yearly.keys()):
    marker = " **" if abs(strat_yearly[yr]) > abs(spy_yearly[yr]) else ""
    print(f"{yr:<8} {spy_yearly[yr]:>9.1f}% {smh_yearly[yr]:>9.1f}% {vgt_yearly[yr]:>9.1f}% {tqqq_yearly[yr]:>9.1f}% {strat_yearly[yr]:>11.1f}%{marker}")
print("=" * 75)
print()
...
print("=" * 75)
print(f"{'Metric':<15} {'SMH':>10} {'VGT':>10} {'TQQQ':>10}")
print("-" * 45)
print(f"{'Mean':<15} {df['allocation_SMH'].mean():>9.1f}% {df['allocation_VGT'].mean():>9.1f}% {df['allocation_TQQQ'].mean():>9.1f}%")
print(f"{'Median':<15} {df['allocation_SMH'].median():>9.0f}% {df['allocation_VGT'].median():>9.0f}% {df['allocation_TQQQ'].median():>9.0f}%")
print()
...
print(f"  Map A (no TQQQ):  {map_a_days:>6} days  ({map_a_days/len(df)*100:.1f}%)")
